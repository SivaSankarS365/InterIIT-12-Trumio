...
class InterviewAgent:
    def __init__(self, topic, keywords):
        self.model = Model(
            GPT_3_3, system=self.make_sys_interviewer(topic, keywords), return_json=True
        )
    ...
    def __call__(self, msg):
        ...
        if len(self.history) > self.max_len:
            self.next_sys = (
                "You can stop the interview. Put a thank you note. Say TERMINATE"
            )
    ...
    def get_feedback(self, subtopics, model_name=GPT_3_4):
    ...
    """
        ...
        feedback = json.loads(
            feedback_model(
                prompt.format(history=history, subtopics=json.dumps(subtopics))
            )
        )
        return feedback
