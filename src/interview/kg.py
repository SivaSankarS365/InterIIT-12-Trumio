...
class SubGrapher:
    ...
    def get_similar_nodes(self, resume_vec):
        ...
        similarities = F.threshold(
            self.cosine(resume_vec, self.node_encoding), # 768*1, 768*N => 1*N => N
            self.threshold,
            0,
        )
        return np.where(similarities >= self.threshold)[-1] + 1 # 0,1,2,3 => 1,2,3,4 | 
        ...
        for topic_node in self.topic_nodes:
            ...
            for edge in self.edges:
                if (
                    ...
                    proficiency[topic].append(self.nodes[int(edge["target"]) - 1])
