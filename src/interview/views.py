...
def pdf_upload_view(request, *args, **kwargs):
    ...
    try:
        ...
        return Response(
            data=dict({"error": f"Missing key : {str(e)}"}),
            status=status.HTTP_400_BAD_REQUEST,
        )

    except Exception as e:
        return Response(
            data=dict({"error": f"Something went wrong: {str(e)}"}),
...
def get_relevant_profile(request, *args, **kwargs):
    ...
    try:
        ...
        preferred_profiles = store.get_collection("profiles").query(
            query_embeddings=pr["embeddings"][0], n_results=3
        )

        data = dict(
            zip(preferred_profiles["ids"][0], preferred_profiles["distances"][0])
        )
...
def get_relevant_projects(request, *args, **kwargs):
    ...
    try:
        ...
        if not sid:
            return Response(
                {"error": "Missing sid parameter"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        preferred_projects = store.get_collection("projects").query(
            query_embeddings=sp["embeddings"][0], n_results=15, include=["distances"]
        )

        data = dict(
            zip(preferred_projects["ids"][0], preferred_projects["distances"][0])
        )
...
def get_relevant_projects_for_team(request, *args, **kwargs):
    ...
    try:
        ...
        preferred_projects = store.get_collection("projects").query(
            query_embeddings=tp["embeddings"][0], n_results=15, include=["distances"]
        )
        data = dict(
            zip(preferred_projects["ids"][0], preferred_projects["distances"][0])
        )
...
def get_relevant_teams(request, *args, **kwargs):
    ...
    try:
        ...
        preferred_teams = store.get_collection("teams").query(
            query_embeddings=pr["embeddings"][0],
            n_results=15,
...
def get_scores(request, *args, **kwargs):
    ...
    try:
        ...
        student_embed = store.get_collection("profiles").get(
            ids=[sid], include=["embeddings"]
        )["embeddings"]
